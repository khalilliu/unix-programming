...
int main(int ac, char *av[])
{
  ...
  while ((msglen = recvfrom(server_fd, buf, BUFSIZ, 0,
                            (struct sockaddr *)&client_address,
                            &address_len)) > 0)
  {
    printf("dgrecv: got a message: %s\n", buf);
    say_who_called(&client_address);
    reply_to_sender(server_fd, buf, &client_address, address_len);
  }
}
...
void reply_to_sender(int server_fd, char *msg, struct sockaddr_in *addrp, socklen_t len)
{
  char reply[BUFSIZ + BUFSIZ];
  sprintf(reply, "Thanks for your %ld char message\n", strlen(msg));
  sendto(server_fd, reply, strlen(reply), 0, addrp, len);
}